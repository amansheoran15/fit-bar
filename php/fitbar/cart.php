...
<?php
    ...
    if(isset($_POST["remove"])){
        if($_GET['action']=="remove"){
                    ...
                    include "dbconnect.php";
                        ...
                        $prid= $_POST['pid2'];
                        $sizep= $_POST['productsize2'];
                        $sql="DELETE FROM `cart` WHERE `prod_id` = '$prid' AND `size` = '$sizep' AND `username` = '$uname'";
    ...
    if(isset($_POST['size'])){
        ...
        $uname = $_SESSION['username'];
        $idd= $_POST['pid3'];
        $sqlxx="SELECT * FROM `cart` WHERE `username`='$uname' AND `prod_id`='$idd' AND `size`='$sizee'";
        $resultxx= mysqli_query($conn,$sqlxx);
        $numxx=mysqli_num_rows($resultxx);
        if($numxx>0){
            echo '<div class="alert alert-danger m-0 alert-dismissible fade show" role="alert">
                    <strong>Alert!</strong> Product is already in cart.
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </div>';
        }
        else{
        ...
        while($row=mysqli_fetch_assoc($result2)){
            if($row['prod_id']==$idd){
                $srno=$row['sr'];
                $sql4="UPDATE `cart` SET `size` = '$sizee' WHERE `cart`.`sr` = $srno";
            ...
            <div class="col-md-7">
                <div class="shopping-cart">
                    ...
                    if(isset($_POST['quant'])){
                        ...
                        $idd= $_POST['pid'];
                        $qt=$_POST['quant'];
                        $sizee=$_POST['productsize'];
                        include "dbconnect.php";
                        $sql4="UPDATE `cart` SET `quantity` = '$qt' WHERE `prod_id` = '$idd' AND `size`='$sizee' AND `username`='$uname' ";
                        $result4= mysqli_query($conn,$sql4);
                    ...
                    if(isset($_SESSION['cart'])){
                        ...
                        while($rows=mysqli_fetch_assoc($result)){
                            ...
                            if($rows['id']==$product_id){
                                ...
                                cartElement($rows['product_image'],$rows['product_name'],$rows['product_price'],$rows['id'],$quanti,$size);
